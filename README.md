...
| AMD Radeon 660M iGPU     | RDNA2               |   6 CU  |       82 Mkey/s  |

### CPUs (x86-64)

| Device                          | Architecture          | Threads |   Speed (keys/s) |
|---------------------------------|-----------------------|---------|------------------|
| XEON PLATINUM 8568Y+  |   Emerald Rapids  |  96          | 360 Mkey/s |
...
The `_cuda` and `_gpu` builds load the NVIDIA CUDA driver
(`libcuda.so.1` on Linux, `nvcuda.dll` on Windows) dynamically at run
time — there is no link-time dependency, so the binary still starts on
a machine that has no NVIDIA driver and simply runs without CUDA. If
the driver is installed in a non-standard location, set the
`BM_CUDA_LIB` environment variable to the full path of that library
